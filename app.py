...
import os
import numpy as np
# --- Compatibilidad psycopg2 <-> psycopg (psycopg3) ---
try:
    import psycopg  # psycopg3 (lo que instala Streamlit Cloud)
    from psycopg import connect as _pg3_connect

    class _Psycopg2Compat:
        @staticmethod
        def connect(**kwargs):
            # psycopg3 usa 'dbname'; si alguien envía 'database', lo convertimos
            if 'database' in kwargs and 'dbname' not in kwargs:
                kwargs['dbname'] = kwargs.pop('database')
            return _pg3_connect(**kwargs)

    psycopg2 = _Psycopg2Compat  # alias con misma interfaz .connect()
except Exception:
    import psycopg2  # si existe psycopg2 real (tu entorno local), úsalo
# --- Fin compatibilidad ---
